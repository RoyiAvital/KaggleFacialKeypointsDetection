...
hogCellSize     = 8;
hogBlockSize    = 2;
blockOverlap    = ceil(hogBlockSize / 2);
hogNumBins      = 9;
...
tHogDescriptor = ExtractHogDescriptor(tRefImages, hogCellSize, hogBlockSize, blockOverlap, hogNumBins, OFF);
...
mDataSamples    = [mDataSamples; permute(tHogDescriptor, [2, 3, 1])];
...
mPredFeatureCoord = hRegNet([mTestImage(:); extractHOGFeatures(mTestImage, ...
...
if(runTest)
    ...
    tHogDescriptor = ExtractHogDescriptor(tTestImage, hogCellSize, hogBlockSize, blockOverlap, hogNumBins, OFF);
